...
export default function SearchModal({ isOpen, onClose, onSelectQuestion }) {
  ...
  const handleSearch = useCallback((val) => {
    setQuery(val);
    if (val.length >= 2) {
      setResults(searchQuestions(val));
    } else {
      setResults([]);
    }
  }, []);
          ...
          <div className="max-h-72 overflow-y-auto scrollbar-hide">
            {query.length >= 2 && results.length === 0 && (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-slate-600 text-xs mt-1">Try different keywords or browse sections below</p>
              </div>
            )}
                ...
                {results.map(q => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-slate-900 truncate">{q.question}</p>
            ...
            {query.length < 2 && recent.length === 0 && (
              <div className="px-4 py-6 text-center text-slate-500 text-sm">
                Start typing to search across all FAQs
              </div>
            )}
          </div>
        </div>
