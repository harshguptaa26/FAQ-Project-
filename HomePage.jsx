        ...
        {timelineSteps.map((step, idx) => (
          <div key={idx} className="relative flex gap-5">
            ...
            <div className="flex flex-col items-center flex-shrink-0 z-10">
              <div className="w-10 h-10 rounded-full bg-white border-2 border-slate-200 flex items-center justify-center text-[11px] font-bold text-slate-500 shadow-sm">
                {idx + 1}
              </div>
            </div>

            {/* Card */}
            <div className="flex-1 bg-white border border-slate-100 rounded-xl p-4 mb-3 shadow-sm">
...
export default function HomePage({ onSearchOpen }) {
  ...
  const totalViews = faqData.reduce((a, c) => a + c.questions.reduce((b, q) => b + (q.clicks || 0), 0), 0);
  ...
  return (
    <div className="min-h-screen font-sans" style={{ backgroundColor: '#FAF6EF', paddingTop: '56px' }}>
      ...
      {/* ── Panic Alert ── */}
      {!panicDismissed && (
        <div className="bg-[#c0392b] text-white px-5 py-2.5 flex items-center gap-2.5 text-sm">
          <AlertTriangle size={14} className="flex-shrink-0 text-white/90" />
          <span>
            <span className="font-semibold">NOC Panic Alert.</span>{' '}
            Searches for 'NOC Dates' jumped from 22 to 96 today. NOC questions have been moved to the top of the portal.
          </span>
          <button onClick={() => setPanicDismissed(true)} className="ml-auto text-white/70 hover:text-white p-0.5 rounded">
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-8 py-8 space-y-8">
        ...
        {/* ── Section label ── */}
        <div>
          <p className="text-[11px] font-bold tracking-[0.18em] text-amber-600 uppercase mb-1">Overview</p>
          <h1 className="text-2xl font-bold text-slate-900">Programme Dashboard</h1>
          <p className="text-sm text-slate-500 mt-1 font-normal">Your complete guide to the Vicharanashala internship programme</p>
        </div>
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
          ...
          <button
            ...
            className="mt-4 flex items-center gap-2 text-xs text-amber-700 border border-amber-200 bg-amber-50 px-4 py-2 rounded-full hover:bg-amber-100 transition-colors font-medium"
          >
                  ...
                  {faqData.map(section => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-slate-50 group transition-colors"
                      >
                        <span className={`${meta.iconColor} flex-shrink-0`}>{meta.icon}</span>
